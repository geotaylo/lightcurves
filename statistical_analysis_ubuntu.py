...
- For some reason my ubuntu machine won't upgrade past python 2.7.12, so this is a 'deprecated' version of the
 statistical analysis script, for running on the old machine.
...
def plot_errors_norm(scopes, labels, colour, folder):
    ...
    try:
        ...
        for i in range(0, len(scopes)):
            ...
            data = ax1.hist(trimmed_c, bins_c, histtype='stepfilled', normed=True, color=colour[i],
                            label=labels[i]+'(%s fits)'%str(len(trimmed_c)), alpha=0.2)
            ...
            ax3.hist(trimmed_c, bins_c, histtype='stepfilled', normed=True, color=colour[i], label=labels[i], alpha=0.2)
            ax3.plot(x_fit, y_fit, lw=2, color=colour[i])
    ...
    try:
        ...
        for i in range(0, len(scopes)):
            ...
            # Plot error histograms on top plot
            data = ax4.hist(trimmed_t0, bins_t0, histtype='stepfilled', normed=True, color=colour[i],
                            label=labels[i] + '(%s fits)' % str(len(trimmed_t0)), alpha=0.2)
            ...
            # Plot data and fit overlaid on bottom plot
            ax6.hist(trimmed_t0, bins_t0, histtype='stepfilled', normed=True, color=colour[i], label=labels[i], alpha=0.2)
            ax6.plot(x_fit, y_fit, lw=2, color=colour[i])
